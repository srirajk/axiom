...
/** Single test-only authority for Axiom's repository-owned platform-policy bundle and Cerbos image. */
public final class AxiomTestPolicyRoot {
    ...
    public static Path policies() {
        ...
        for (int depth = 0; depth < 6 && cursor != null; depth++, cursor = cursor.getParent()) {
            Path candidate = cursor.resolve("platform-policy/policies");
            if (Files.isDirectory(candidate)) return candidate;
        }
        throw new IllegalStateException("platform-policy/policies package is unavailable");
    }
